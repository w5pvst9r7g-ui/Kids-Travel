...
### Latest
- [x] **🕹️ Reef Dex GO — Pokémon-GO look & feel with realistic silhouettes** — all 63 creatures now have **hand-tuned, render-verified SVG silhouettes** (GO-style solid shapes — the lionfish's fan spines, the manta's cephalic horns, the seahorse curl — no clipart, no emoji as primary art). Dex cards became **GO-style art boxes**: solid silhouette until caught, real photo + **PWR chip** (HP + move powers) once caught. New **"On the reef near you" tray**: six mystery silhouettes with pulsing radar rings (species you haven't caught, commons first, rotates daily) — tap to study the trading card, then go find the real one. The catch splash gained GO-style expanding encounter rings + PWR readout; trading cards use the silhouette when locked. Night-mode silhouette fills, reduced-motion kill-switch, and a smoke-test assertion that every species has a valid path.
- [x] **🎴 Reef Dex v3 — 63 creatures, 5 regions, trading cards & expert IDs** — the dex grew to **63 research-verified species** organised into **5 Pokémon-style regions with generation labels**: Lagoonto (Gen 1 — the famous starters), Coraljo (Gen 2 — the coral city), Deepenn (Gen 3 — the big blue), Shadowsinn (Gen 4 — tricksters & hidden treasures) and Islandova (Gen 5 — dry land). Every creature carries **verified Wikimedia Commons photos** (wmURL pipeline, emoji fallback offline) and a **TCG-style trading card** — one of 9 types (Splash/Chomp/Zoom/Armour/Toxic/Stealth/Muncher/Mystic/Island), HP, two named powers, flavour fact, rarity symbol (● ◆ ★ 🌟) and card number; cards are **mystery silhouettes with hidden powers until YOU spot the real creature**, and Legendary/Shiny cards get a gold holo frame. 16 species have **exact-kind photo galleries** (45+ kinds: 5 butterflyfish, 4 triggerfish, 3 sharks, 3 turtles, 3 rays, 2 anemonefish, the Moorish-idol-vs-bannerfish look-alike lesson…) — the log flow asks "🔬 Which one exactly?" with how-to-tell hints, and every new exact ID grows a kid's **expert score** (versus-card stat + Junior Marine Biologist / Professor of the Reef badges). Safety cards teach real rules: sea urchin, moon jelly, cone shell, titan triggerfish, lionfish. Cooldowns are per exact option, so "Butterflyfish at breakfast" can still become "Threadfin — identified!" after studying the photos.
- [x] **🤿 Reef Dex — The Great Kandima Creature Hunt** (headline feature for the Maldives trip) — a Pokédex-style marine-spotting game with rarity tiers **Common/Uncommon/Rare/Legendary** (+ a ✨Shiny✨ glowing-plankton), kid nicknames ("The Real-Life Nemo", "Finn the Friendly", "The Mothership") and a wow-fact per species (incl. real safety tips for titan triggerfish & lionfish). **Scoring:** each species scores its tier points once per kid; the family's first spotter earns a permanent 👑 crown + 1 bonus point; guests log for fun but never score. **Per-kid dex reveal** — creatures stay mystery silhouettes until YOU spot them, so one kid can't spoil the other's hunt. A **Great Creature Race** versus card (avatars, points, firsts, tug-of-war bar) keeps Jean-Luc & Hailey competing. New top-level **Reef view** in both navs (appears 30 days before the trip, stays once used), a Reef Dex tab on the Maldives guide, an Explore banner with live standings, a trip-time 🤿 quick-log FAB, a 3-step log modal (focus-trapped, Esc-registered), a ray-burst **catch splash** for Rares/Legendaries/family-firsts, a recent-sightings feed and **9 new reef badges**. Shared store `globie_atlas_reefdex_v1` (like the cat atlas) with **id-union cloud-restore merge**; all competitive numbers derived from the log (merge-proof). Animated CSS-only ocean hero (waves, bubbles), night-mode-aware silhouettes, reduced-motion kill-switch, 8-tab mobile squeeze.
- [x] **✈️ Maldives trip data corrected to the real booking** — t27 now 16 nights (23 Jul → 8 Aug, was wrongly "9 August"), with an idempotent fix-up for journals that stored the old copy; Getting-there shows the booked Etihad itinerary (EY48/EY376 out, EY379/EY47 home) plus a new claymorphic **booked-flights strip** (generic `d.flights`); Upcoming card gets a wave-pulse countdown chip ≤30 days out; the Maldives hero shimmers with caustic light.
- [x] **📸 Cat photo slideshow** — a full-screen, auto-playing gallery of every cat photo in the World Cat Atlas. Cinematic dark backdrop, Ken Burns zoom on each photo, a caption overlay (coat · type · country · who · date), an auto-advance progress bar (4s/slide), play/pause + prev/next controls, clickable dots, keyboard support (←/→/space/Esc) and a "📸 Slideshow (N)" button on the Cat Atlas page. Newest photos first; focus-trapped and reduced-motion aware.
- [x] **🐱 Cat-spotting is now a headline feature — the World Cat Atlas page** — a brand-new top-level view (in the desktop top nav AND the mobile tab bar, with a custom cat icon) plus a **landing-page banner** on Explore showing live stats ("N cats · N points · N/9 coats · N countries"). The page gathers everything cat in one rich home: a **rank hero** with next-rank goal; a 6-tile stat board (cats, points, countries, mapped, photos, coats); big **Spotted a cat!** / **Open the map** actions; a **family cat-spotting league** leaderboard (who's spotted the most, across explorers); a **coat collection "Catdex"** (collect all 9 coats, locked/greyed until found); **cats by country** bars (tracking sightings across trips); a **recent-sightings feed** with photo thumbnails, who & where; the full stats panel; and a **cat-badge shelf**. Empty-state nudges first-timers to log their first cat.
- [x] **3 new cat badges** — 🌈 Coat Collector (all 9 coats), 🗺️ Globe-trotting Cat Finder (cats in 3 countries), 📸 Cat Photographer (snap a cat photo). Existing cat badges re-worded from "in Morocco" to worldwide now that the atlas is global.
...
- [x] **Removed the made-up cat locations** — on second thought, dropped the auto-placed (approximate) coordinates; only cats whose location was actually recorded via GPS keep a pin. A cleanup pass strips any leftover approx coords from saved data on load (counts/points/badges untouched); new cats only get a pin from real geolocation.
- [x] **Fix: cat pins were landing in the ocean** — replaced the line-zones (which strayed off the mosque/marina into the Atlantic) with hand-picked ON-LAND points (medina alleys, Bd des Almohades esplanade, marina-side roads, Rabat medina) + only ±22 m jitter. Versioned re-placement corrects already-saved approx pins on next load; real GPS pins are never moved. Verified every approx pin sits within ~28 m of a known land spot.
- [x] **Backfill missing cat locations** — cats logged without GPS are now auto-placed (deterministically, persisted) across the family's real Morocco spots: mostly the Casablanca medina, along the Hassan II Mosque → marina-mall corridor, a few on the Casa-Port station → mosque drive, and a few in the Rabat medina. Backfilled pins show dashed + "around here"; real GPS still overrides them.
- [x] **Cat-spotting map** — logging a cat now records the exact GPS location (device geolocation, with permission) onto that sighting. A "🗺️ See your cat map" button in the Cat Counter opens a full-screen Leaflet map with a custom cat pin per spot (coat-colour teardrop + 🐱/🐈), fit to bounds, tappable popups (coat / type / date). Located sightings get a 📍 chip; graceful fallbacks when location is denied or the map can't load; focus-trapped + Esc to close.
- [x] **Cat badges up to 300 + over-the-top stats** — added 😹 Cat-trillionaire (150), 🐾 Nine-Lives Legend (200), 🦁 Big-Cat Boss (250), 🏆 Meow-narch of Morocco (300). New "📊 Your cat statistics" panel in the Cat Counter: a cat rank with next-rank goal, mini stats (cats/points/avg/coats seen), a big-vs-kitten split bar, favourite + rarest coat, a 9-row coat-colour bar chart, and a gleefully silly "Just for fun" section (nose-to-tail metres, whiskers, naps/day, baskets, purr-power, sardines). Per-colour counts now tracked in the data model.
